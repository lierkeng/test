 ...
 * data     ：2016/12/16
 * time     ：18:32
 * function :正则工具类 提供验证邮箱、手机号、电话号码、身份证号码、数字等方法
 */

public final class UtilsRegex {
    /**
     * demo 判断手机号密码和验证码是否正确
     *
     * @param phone
     * @param password
     * @param code
     * @return
     */
    private static boolean checkInput(String phone, String password, String code) {
        if (TextUtils.isEmpty(phone)) { // 电话号码为空
            UtilsToast.showShort("手机号码不能为空");
        } else {
            if (!checkMobile(phone)) { // 电话号码格式有误
                UtilsToast.showShort("手机号码格式不正确");
            } else if (TextUtils.isEmpty(code)) { // 验证码不正确
                UtilsToast.showShort("请输入验证码");
            } else if (password.length() < 6 || password.length() > 32
                    || TextUtils.isEmpty(password)) { // 密码格式
                UtilsToast.showShort("请输入6-32个密码");
            } else {
                return true;
            }
        }
